...
export async function GET(request:Request) {
 ...
 const issues=(await env.NEWSLETTER_DB.prepare('SELECT id,slot,status,title,slug,attempts,email_sent_at,email_id,error FROM newsletter_issues ORDER BY created_at DESC LIMIT 5').all<{id:string;email_id:string|null}>()).results;
