...
export type EditorialPhoto={url:string;alt:string;caption?:string;credit?:string;source?:string;license?:string;licenseUrl?:string};
export default function PostThumbnail({photo,href,priority=false}:{photo:EditorialPhoto;href?:string;priority?:boolean}) {
 const picture=<div className="overflow-hidden rounded-sm bg-[#122535]"><img data-post-thumbnail src={photo.url} alt={photo.alt} width="1000" height="667" loading={priority?'eager':'lazy'} className="aspect-[3/2] w-full object-contain transition-transform duration-500 group-hover:scale-[1.025]"/></div>;
