...
export function ScreenshotImport({ onImport, suggestedUrl = '' }: { onImport: (yacht: YachtListing, side: 'a' | 'b') => void; suggestedUrl?: string }) {
  ...
  useEffect(() => setCanCapture(Boolean(navigator.mediaDevices?.getDisplayMedia)), []);
  ...
  const run = async () => {
    ...
    try {
      ...
      setSavedLink(data.savedImportId ? `${location.origin}/compare?import=${data.savedImportId}` : '');
  ...
  return <div className="mb-6 rounded-xl border border-border bg-bg-card p-4">
    ...
    {!open && <p className="mt-1 text-xs text-text-secondary">If a site blocks the link, upload screenshots of its boat photo and specs.</p>}
    {savedLink && <div className="mt-3 text-sm"><a href={savedLink} className="text-gold underline">Open saved yacht</a><p className="mt-1 text-xs text-text-secondary">Copy this link to open the captured listing on another device.</p><input aria-label="Saved yacht link" readOnly value={savedLink} onFocus={event => event.target.select()} className="mt-2 w-full rounded border border-border bg-bg-secondary p-2 text-xs" /></div>}
    {open && <div className="mt-4 space-y-4">
      <p className="text-sm text-text-secondary">Open the listing and take screenshots of its boat photo, title, price and specifications. Upload them below.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded border border-border p-3"><p className="text-sm font-medium">1. Boat photo</p><p className="mb-2 text-xs text-text-secondary">Open the gallery photo before capturing, or upload the photo itself.</p>
          {canCapture && <button type="button" disabled={busy} onClick={() => capture('photo')} className="mb-2 min-h-10 text-sm text-gold">Capture photo tab</button>}
        ...
        <div className="rounded border border-border p-3"><p className="text-sm font-medium">2. Specification screenshots</p><p className="mb-2 text-xs text-text-secondary">Include the title and price. Expand the specs and engine sections. Up to 4 images.</p>
          {canCapture && <button type="button" disabled={busy || specs.length >= 4} onClick={() => capture('spec')} className="mb-2 min-h-10 text-sm text-gold">Capture specs tab</button>}
