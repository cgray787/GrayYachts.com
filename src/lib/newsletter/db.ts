...
export async function publishedIssues(): Promise<Issue[]> {
 ...
 return (await env.NEWSLETTER_DB.prepare("SELECT id,slot,status,title,slug,audience,excerpt,hero,images,created_at,published_at FROM newsletter_issues WHERE status='published' ORDER BY published_at DESC").all<Issue>()).results;
}
