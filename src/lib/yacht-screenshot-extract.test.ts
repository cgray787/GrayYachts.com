...
describe('screenshot specification import', () => {
  it('sends multiple images and leaves missing specifications unknown', async () => {
    ...
    const fetcher = vi.fn(async () => Response.json({ content: [{ type: 'text', text: JSON.stringify({ name: '2007 Sabre 34', builder: 'Sabre', year: 2007, lengthFt: 34, cabins: 1, price: 'US$200,000', priceNum: 200000, engineHours: 'made up', maxSpeed: -5 }) }] }));
    ...
    expect(result?.engineHours).toBeNull(); expect(result?.maxSpeed).toBeNull(); expect(result?.range).toBeNull();
