...
The first existing pending draft can receive its images through authenticated `POST /api/newsletter/images`; this cannot replace an existing photo set or change a reviewed issue. It increments the review revision, invalidating the older approval token and requesting a new review email with a distinct idempotency key. New approvals are tied to the stored revision. The first selection is Seawulff exterior, Neverland helm and Playa Linda salon. Twelve tests now include image-count validation, duplicate rejection, cross-edition atomic reservations, rejection retention and old-link invalidation.

## Licensed imagery and art direction

Seven commercially licensed Pexels images extend the catalog to 131 images. See `newsletter-photo-licenses.md` and `newsletter-licensed-images.json` for provenance and usage restrictions. Existing ledger reservations still apply. The scheduled context rotates suggested perspectives across editions; Hermes must adapt those suggestions to the topic and available unused photos. Prefer a sharp landscape lead, one or two complementary photos, accurate restrained captions and coherent light/color. Reject poor images and screenshot controls. The email distributes images through the article instead of placing every photo at the top. Two strong photos are preferable to a weak third.
